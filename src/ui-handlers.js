(function(root){
  'use strict';
  const tradeUtils = root.tradeUtils || require('./trade-utils');
  ...
  function updateEndDateMin(index, leg){
    ...
    if(!start || !end) return;
    const d = tradeUtils.parseInputDate(start.value);
    if(!d) return;
  ...
  function updateAvgRestrictions(index){
    ...
    const endDate = tradeUtils.parseInputDate(end.value);
    if(!endDate) return;
    ...
    const currentIdx = tradeUtils.monthNames.indexOf(monthSel.value);
    if(currentIdx < 0 || monthSel.options[currentIdx].disabled){
      for(let i=limitMonth;i<monthSel.options.length;i++){
        if(!monthSel.options[i].disabled){
          monthSel.value = tradeUtils.monthNames[i];
          break;
        }
      }
    }
  ...
  function generateRequest(index){
    ...
    try {
      ...
      const dateFix1 = dateFix1Raw ? formatDate(tradeUtils.parseInputDate(dateFix1Raw)) : '';
      const dateFix2 = dateFix2Raw ? formatDate(tradeUtils.parseInputDate(dateFix2Raw)) : '';
      ...
      if(leg1Type.startsWith('AVG')){
        avgMonth = month; avgYear = year;
      } else if(leg2Type.startsWith('AVG')){
        avgMonth = month2; avgYear = year2;
      }
      const monthIndex = avgMonth ? new Date(`${avgMonth} 1, ${avgYear}`).getMonth() : 0;
      const pptDateAVG = avgMonth ? tradeUtils.getSecondBusinessDay(avgYear, monthIndex) : '';

      let leg1;
      const showPptAvg = (leg2Type === 'Fix' && dateFix2Raw && !useSamePPT2) ||
                         (leg1Type === 'Fix' && dateFix1Raw && !useSamePPT1);
      if(leg1Type === 'AVG'){
        leg1 = `${tradeUtils.capitalize(leg1Side)} ${q} mt Al AVG ${month} ${year}`;
        if(showPptAvg) leg1 += ` ppt ${pptDateAVG}`;
        leg1 += ' Flat';
      } else if(leg1Type === 'AVGInter'){
        const start = tradeUtils.parseInputDate(startDateRaw);
        const end = tradeUtils.parseInputDate(endDateRaw);
        if(!start || !end) throw new Error('Start and end dates are required for AVG Inter.');
        const startStr = formatDate(start);
        const endStr = formatDate(end);
        leg1 = `${tradeUtils.capitalize(leg1Side)} ${q} mt Al Fixing AVG ${startStr} to ${endStr}`;
        if(showPptAvg) leg1 += ` ppt ${pptDateAVG}`;
      } else {
        let pptFixLeg1;
        if(useSamePPT1){
          leg1 = `${tradeUtils.capitalize(leg1Side)} ${q} mt Al USD ppt ${pptDateAVG}`;
        } else {
          try {
            pptFixLeg1 = tradeUtils.getFixPpt(dateFix1);
          } catch(err){
            err.fixInputId = `fixDate1-${index}`;
            throw err;
          }
          leg1 = `${tradeUtils.capitalize(leg1Side)} ${q} mt Al USD ppt ${pptFixLeg1}`;
        }
      }

      let leg2;
      if(leg2Type === 'AVG'){
        leg2 = `${tradeUtils.capitalize(leg2Side)} ${q} mt Al AVG ${month2} ${year2}`;
        if(showPptAvg) leg2 += ` ppt ${pptDateAVG}`;
        leg2 += ' Flat';
      } else if(leg2Type === 'AVGInter'){
        const start = tradeUtils.parseInputDate(document.getElementById(`startDate2-${index}`)?.value || '');
        const end = tradeUtils.parseInputDate(document.getElementById(`endDate2-${index}`)?.value || '');
        if(!start || !end) throw new Error('Start and end dates are required for AVGInter.');
        const sStr = formatDate(start);
        const eStr = formatDate(end);
        leg2 = `${tradeUtils.capitalize(leg2Side)} ${q} mt Al Fixing AVG ${sStr} to ${eStr}`;
        if(showPptAvg) leg2 += ` ppt ${pptDateAVG}`;
      } else if(leg2Type === 'Fix'){
        let pptFix;
        if(useSamePPT2){
          leg2 = `${tradeUtils.capitalize(leg2Side)} ${q} mt Al USD ppt ${pptDateAVG}`;
        } else {
          try {
            pptFix = tradeUtils.getFixPpt(dateFix2);
          } catch(err){
            err.fixInputId = `fixDate-${index}`;
            throw err;
          }
          leg2 = `${tradeUtils.capitalize(leg2Side)} ${q} mt Al USD ppt ${pptFix}`;
        }
        ...
        leg2 = `${tradeUtils.capitalize(leg2Side)} ${q} mt Al C2R ${dateFix2} ppt ${pptFix}`;
      }
  ...
  function syncLegSides(index, changedLeg){
    ...
    if(changedLeg === 1){
      ...
      const other = document.querySelector(`input[name='side2-${index}'][value='${opposite}']`);
      if(other) other.checked = true;
    } else if(changedLeg === 2){
      const opposite = leg2.value === 'buy' ? 'sell' : 'buy';
      const other = document.querySelector(`input[name='side1-${index}'][value='${opposite}']`);
      if(other) other.checked = true;
    } else {
      const opposite = leg1.value === 'buy' ? 'sell' : 'buy';
      const other = document.querySelector(`input[name='side2-${index}'][value='${opposite}']`);
  ...
  function toggleLeg1Fields(index){
    ...
    if(fixInput && fixInput.parentElement)
      fixInput.parentElement.style.display = val === 'Fix' || val === 'C2R' ? '' : 'none';
    ...
    if(samePpt && samePpt.parentElement){
      ...
      if(showChk && samePpt.checked){
        ...
        const date = tradeUtils.getSecondBusinessDay(year, monthIdx);
        fixInput.value = calendarUtils.parseDateGregorian(date).toISOString().split('T')[0];
      }
    }
  }
  ...
  function toggleLeg2Fields(index){
    ...
    if(samePpt && samePpt.parentElement){
      const showChk = val === 'Fix' && type1 === 'AVG';
      samePpt.parentElement.style.display = showChk ? '' : 'none';
      if(!showChk) samePpt.checked = false;
      if(showChk && samePpt.checked){
        const month = document.getElementById(`month1-${index}`).value;
  ...
  function addTrade(){
    ...
    document.getElementById(`samePpt1-${index}`).addEventListener('change', () => toggleLeg1Fields(index));
    document.getElementById(`samePpt2-${index}`).addEventListener('change', () => toggleLeg2Fields(index));
